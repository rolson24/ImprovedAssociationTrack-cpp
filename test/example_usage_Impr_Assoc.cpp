...
// // Initialize GlobalMotionCompensation
    ...
    */
    ...
    std::unique_ptr<ImprAssocTracker> tracker;
    ...
    if (argc == 4)
    {
        std::cout << "Using default configs. No ReID..."
                  << std::endl;
        tracker = std::make_unique<ImprAssocTracker>(fps, track_buffer, "", "");
    }
    else
    {
        tracker = std::make_unique<BoTSORT>(fps, track_buffer, reid_config_path,
                                            reid_onnx_model_path);
    }
